def fiveMinDemand():
    #total_orders = orders + orders2 tables minus duplicates from overlap
    #SELECT CONVERT_TZ(delivered_at,'+00:00','-05:00') as time_interval, COUNT(delivered_at) as count FROM total_orders where delivered_at <> '0000-00-00 00:00:00' and delivered_at > '2017-11-01 00:00:00' GROUP BY date(delivered_at), hour(delivered_at), minute(delivered_at),second(delivered_at);
    
    cursor.execute("SELECT CONVERT_TZ(delivered_at,'+00:00','-05:00') as time_interval, COUNT(distinct id) as count FROM orders where delivered_at <> '0000-00-00 00:00:00' and delivered_at > '2017-11-01 00:00:00' GROUP BY date(delivered_at), hour(delivered_at), minute(delivered_at),second(delivered_at);")
    ...
    df = df.resample('5min').sum() #number of deliveries in 5 min intervals
    #print(df)
    df.to_csv('temp.csv')
    x = 2880 #period
    y = "five" #for saving .csv filename
    z = 0.01 #changepoint_prior_scale (flexibility)
    prophetForecast(x,y,z)
    return
...
def thirtyMinDemand():
    ...
    df['time_interval'] = pd.to_datetime(df['time_interval'])
    df.index=df['time_interval']
    df = df.resample('30min').sum() #number of deliveries in 30 min intervals
    #print(df)
    df.to_csv('temp.csv')
    x = 2880 #period
...
def prophetForecast(x,y,z):
    df = pd.read_csv('temp.csv') #read temp.csv
    w_df = pd.read_csv('weather_dates.csv') #read inclemnt weather dates
    ...
    'ds': pd.to_datetime(['2018-02-06 12:00:00', '2018-02-06 15:00:00', 
                          '2018-02-06 18:00:00', '2018-02-06 21:00:00']), #day with freezing rain
    'lower_window': -.125,
    'upper_window': 0,
    })
...
def getInclementWeather():
    ...
    for i in f._weathers[:8]: #put tommorow's forecasted dates that are within working hours and have certain weather codes in a list
         ...
         if (i.get_reference_time('date').hour >= 15 or i.get_reference_time('date').hour <= 6) and (i.get_weather_code() == '511' or i.get_weather_code() == '502' or i.get_weather_code() == '503' or i.get_weather_code() == '202' or i.get_weather_code() == '212' or i.get_weather_code() == '602'):
...
def combineCSV(): #combine the five min prediction and thirty min prediction to one .csv file
...
def createPrediction_OutputTable(): #create prediction_output table in mySQL and insert data from predictionOutput.csv
    cursor.execute("CREATE TABLE prediction_output (id INT AUTO_INCREMENT PRIMARY KEY, date_and_time DATETIME, five_min_prediction DOUBLE, thirty_min_prediction DOUBLE, five_min_drivers INT, thirty_min_drivers INT);")
    cursor.execute("LOAD DATA LOCAL INFILE 'predictionOutput.csv' INTO TABLE prediction_output COLUMNS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '\"' ESCAPED BY '\"' LINES TERMINATED BY '\n' IGNORE 1 LINES;")
    cnx.commit()
    return

def updatePrediction_OutputTable(): #create temp_table, insert predictionOutput.csv into temp_table, compare temp_table with prediction_output table, update existing entries, insert new entries, drop temp_table.
    cursor.execute("CREATE TEMPORARY TABLE temp_table LIKE prediction_output;")
    cursor.execute("LOAD DATA LOCAL INFILE 'predictionOutput.csv' INTO TABLE temp_table COLUMNS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '\"' ESCAPED BY '\"' LINES TERMINATED BY '\n' IGNORE 1 LINES(@dummy, date_and_time, five_min_prediction, thirty_min_prediction, @dummy, @dummy);")
    cursor.execute("INSERT INTO prediction_output (date_and_time, five_min_prediction, thirty_min_prediction) SELECT date_and_time, five_min_prediction, thirty_min_prediction FROM temp_table WHERE date_and_time NOT IN (SELECT date_and_time FROM prediction_output);")
    cursor.execute("UPDATE prediction_output JOIN temp_table ON prediction_output.date_and_time = temp_table.date_and_time  SET prediction_output.five_min_prediction = temp_table.five_min_prediction, prediction_output.thirty_min_prediction = temp_table.thirty_min_prediction;")
    cursor.execute("DROP TEMPORARY TABLE temp_table;")
    cnx.commit()
    return

def job():
    getInclementWeather() #DO NOT USE MORE THAN 1 TIME PER 10 MINUTES, free api key subject to throttling
    fiveMinDemand()
    thirtyMinDemand()
    combineCSV()
    updatePrediction_OutputTable()
...
import csv #import/export csv files
...
import schedule as sch #pip install schedule , for scheduled running of functions
import time
try:
    cnx = mysql.connector.connect(user='root', password='andrew1', host='127.0.0.1', database='cargo') #connect to mySQL datebase
except:
    print('Connection Failed')
else:
    cursor = cnx.cursor()
    sch.every().day.at("08:00").do(job) #run at 08:00 everyday
    while True:
        sch.run_pending()
        time.sleep(1)
        
#createPrediction_OutputTable()
#getInclementWeather() #DO NOT USE MORE THAN 1 TIME PER 10 MINUTES, free api key subject to throttling
#fiveMinDemand()
#thirtyMinDemand()
#combineCSV()
